...
def genOpenCL(input,ex,ey):
    ...
    f.write('__constant double dx = {};\n\n'.format(input['dx']))
    f.write('__constant double dy = {};\n\n'.format(input['dy']))
    ...
    f.write('        int i_x, i_y;\n\n')
    f.write('        clMirIndex(i,mir_x,mir_y,&i_x,&i_y);    // mirror indexes if required\n\n')
    f.write('        double flux_out = dt*(FLUX_X(ix,iy,i)/dx + FLUX_Y(ix,iy,i)/dy);\n\n')
    f.write('        double flux_in =  dt*(FLUX_X(x_,iy,i_y)/dx + FLUX_Y(ix,y_,i_x)/dy);\n\n')
    ...
    f.write('                    flux_in += inx*F_FLUX_X(xm1,ym1,i)/dx + iny*F_FLUX_Y(xm1,ym1,i)/dy;        // flux into solid\n\n')
